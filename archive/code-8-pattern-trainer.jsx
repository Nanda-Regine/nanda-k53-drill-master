...
export default function PatternTrainer() {
  ...
  // ─── HOME ────────────────────────────────────────────────────────────────────
  if (mode === "home") return (
    <div style={{ minHeight:"100vh", background: BG, fontFamily:"'Courier New', monospace", padding:"24px 16px", color: TEXT }}>
      <div style={{ maxWidth:640, margin:"0 auto" }}>
        <div style={{ textAlign:"center", marginBottom:36 }}>
          ...
          <div style={{ fontSize:32, fontWeight:900, color:"#fff", lineHeight:1.1, marginBottom:8 }}>
            KNOW YOUR<br/><span style={{ color:"#FFD700" }}>NUMBERS</span>
          </div>
        ...
        <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
          ...
          <button onClick={() => setMode("map")}
            style={{ background: CARD, border:`2px solid #FFD700`, borderRadius:4, padding:"20px 20px", textAlign:"left", cursor:"pointer", fontFamily:"'Courier New', monospace" }}>
            <div style={{ color:"#FFD700", fontSize:10, letterSpacing:3, marginBottom:6 }}>MODE 1</div>
          ...
          <button onClick={startQuiz}
            style={{ background: CARD, border:`2px solid #00C896`, borderRadius:4, padding:"20px 20px", textAlign:"left", cursor:"pointer", fontFamily:"'Courier New', monospace" }}>
            <div style={{ color:"#00C896", fontSize:10, letterSpacing:3, marginBottom:6 }}>MODE 2 — 41 QUESTIONS</div>
          ...
          <button onClick={startSpeed}
            style={{ background: CARD, border:`2px solid #FF4444`, borderRadius:4, padding:"20px 20px", textAlign:"left", cursor:"pointer", fontFamily:"'Courier New', monospace" }}>
            <div style={{ color:"#FF4444", fontSize:10, letterSpacing:3, marginBottom:6 }}>MODE 3 — 40 CARDS • 15s PER CARD</div>
        ...
        <div style={{ marginTop:32, background: CARD, border:`1px solid ${BORDER}`, borderRadius:4, padding:"18px 16px" }}>
          <div style={{ color:"#FF4444", fontSize:10, letterSpacing:3, marginBottom:14 }}>⚠️ MOST CONFUSABLE PAIRS — STUDY THESE FIRST</div>
          {[
            ...
            <div key={i} style={{ display:"flex", flexWrap:"wrap", gap:6, marginBottom:8, paddingBottom:8, borderBottom:`1px solid ${BORDER}` }}>
              {row.map((item, j) => (
                <span key={j} style={{
                  ...
                  color: item.startsWith("→") ? DIM : "#FFD700",
  ...
  // ─── PATTERN MAP ─────────────────────────────────────────────────────────────
  if (mode === "map") return (
    <div style={{ minHeight:"100vh", background: BG, fontFamily:"'Courier New', monospace", padding:"20px 16px", color: TEXT }}>
      <div style={{ maxWidth:640, margin:"0 auto" }}>
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:20 }}>
          ...
          <button onClick={() => setMode("home")} style={{ background:"transparent", border:`1px solid ${BORDER}`, color:DIM, fontSize:11, padding:"8px 14px", cursor:"pointer", fontFamily:"'Courier New', monospace", borderRadius:3 }}>← HOME</button>
        </div>
          ...
          {PATTERN_GROUPS.map(group => (
            <div key={group.id} style={{ background: CARD, border:`1px solid ${BORDER}`, borderRadius:4, overflow:"hidden" }}>
              <button
                ...
                style={{ width:"100%", background:"transparent", border:"none", padding:"14px 16px", textAlign:"left", cursor:"pointer", fontFamily:"'Courier New', monospace", display:"flex", alignItems:"center", gap:10 }}
              >
        ...
        <button onClick={() => setMode("home")} style={{ width:"100%", marginTop:20, padding:"13px", background:"#FFD700", color:"#000", border:"none", borderRadius:4, fontSize:12, fontWeight:900, letterSpacing:3, cursor:"pointer", fontFamily:"'Courier New', monospace" }}>
          ← BACK TO HOME
        </button>
      </div>
    </div>
      ...
      <div style={{ minHeight:"100vh", background: BG, fontFamily:"'Courier New', monospace", padding:"20px 16px", color: TEXT }}>
        <div style={{ maxWidth:640, margin:"0 auto" }}>
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:14 }}>
            <div>
              <div style={{ color:"#00C896", fontSize:10, letterSpacing:3 }}>NUMBER DRILL</div>
              <div style={{ color:"#fff", fontSize:13, fontWeight:700 }}>All wrong options are real values</div>
            </div>
            <div style={{ color:"#FFD700", fontSize:22, fontWeight:900 }}>{score}<span style={{ color:DIM, fontSize:14 }}>/{quizQ.length}</span></div>
          </div>

          <div style={{ height:3, background:"#1a1a1a", borderRadius:2, marginBottom:20 }}>
            <div style={{ height:"100%", width:`${progress}%`, background:"#00C896", borderRadius:2, transition:"width 0.3s" }} />
          </div>
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:9, marginBottom:14 }}>
            {q.options.map((opt, i) => {
              ...
              if (answered) {
                if (opt === q.correct) { bc="#00C896"; bg="#001a12"; tc="#00C896"; }
                else if (opt === selected) { bc="#FF4444"; bg="#1a0000"; tc="#FF4444"; }
                else { tc="#2a2a2a"; }
              }
              return (
                <button key={i} onClick={() => handleQuizSelect(opt)}
                  style={{ background:bg, border:`2px solid ${bc}`, borderRadius:4, padding:"14px 16px", textAlign:"left", cursor:answered?"default":"pointer", fontFamily:"'Courier New', monospace", display:"flex", justifyContent:"space-between", alignItems:"center", transition:"border-color 0.1s" }}
                  onMouseEnter={e => { if(!answered) e.currentTarget.style.borderColor="#00C896"; }}
                ...
                >
                  ...
                  {answered && opt === q.correct && <span style={{ color:"#00C896" }}>✓</span>}
                  {answered && opt === selected && opt !== q.correct && <span style={{ color:"#FF4444" }}>✗</span>}
                </button>
              );
            })}
          </div>
          ...
          {answered && (
            <div style={{ background: selected===q.correct?"#001a12":"#1a0000", border:`1px solid ${selected===q.correct?"#00C896":"#FF4444"}`, borderRadius:4, padding:"14px 16px", marginBottom:14 }}>
              <div style={{ color:selected===q.correct?"#00C896":"#FF4444", fontSize:10, letterSpacing:3, marginBottom:6, fontWeight:900 }}>
          ...
          {answered && (
            <button onClick={nextQuiz} style={{ width:"100%", padding:"13px", background:"#00C896", color:"#000", border:"none", borderRadius:4, fontSize:12, fontWeight:900, letterSpacing:3, cursor:"pointer", fontFamily:"'Courier New', monospace" }}>
  ...
  if (mode === "quizresult") {
    ...
    return (
      <div style={{ minHeight:"100vh", background: BG, fontFamily:"'Courier New', monospace", padding:"24px 16px", color: TEXT }}>
        <div style={{ maxWidth:640, margin:"0 auto" }}>
          <div style={{ background: pct>=75?"#001a12":"#1a0000", border:`2px solid ${pct>=75?"#00C896":"#FF4444"}`, borderRadius:4, padding:"28px", textAlign:"center", marginBottom:24 }}>
            <div style={{ color:pct>=75?"#00C896":"#FF4444", fontSize:10, letterSpacing:4, marginBottom:10 }}>NUMBER DRILL RESULT</div>
            <div style={{ color:"#fff", fontSize:52, fontWeight:900, lineHeight:1 }}>{score}<span style={{ color:"#333" }}>/{quizQ.length}</span></div>
            <div style={{ color:pct>=75?"#00C896":"#FF4444", fontSize:22, fontWeight:900, marginTop:4 }}>{pct}%</div>
          ...
          {wrongList.length > 0 && (
            <div style={{ marginBottom:24 }}>
              <div style={{ color:"#FF4444", fontSize:10, letterSpacing:3, marginBottom:14 }}>✗ PATTERNS YOU'RE STILL CONFUSING</div>
              {wrongList.map((w,i) => (
                <div key={i} style={{ background:"#111", border:`1px solid #2a0000`, borderRadius:4, padding:"14px", marginBottom:10 }}>
                  ...
                  <div style={{ color:"#FF4444", fontSize:13, fontWeight:900, marginBottom:4 }}>✗ You chose: {w.yours}</div>
                  <div style={{ color:"#00C896", fontSize:13, fontWeight:900, marginBottom:8 }}>✓ Correct: {w.correct}</div>
          ...
          <div style={{ display:"flex", gap:10 }}>
            <button onClick={startQuiz} style={{ flex:1, padding:"13px", background:"#FF4444", color:"#fff", border:"none", borderRadius:4, fontSize:11, fontWeight:900, letterSpacing:2, cursor:"pointer", fontFamily:"'Courier New', monospace" }}>RETRY</button>
            <button onClick={() => setMode("home")} style={{ flex:1, padding:"13px", background:"#FFD700", color:"#000", border:"none", borderRadius:4, fontSize:11, fontWeight:900, letterSpacing:2, cursor:"pointer", fontFamily:"'Courier New', monospace" }}>HOME</button>
          </div>
        </div>
      </div>
    );
  ...
  if (mode === "speed") {
    ...
    const tColor = timeLeft>8?"#00C896":timeLeft>4?"#FFD700":"#FF4444";
      ...
      <div style={{ minHeight:"100vh", background: BG, fontFamily:"'Courier New', monospace", padding:"20px 16px", color: TEXT }}>
        <div style={{ maxWidth:640, margin:"0 auto" }}>
          <div style={{ display:"flex", justifyContent:"space-between", marginBottom:12 }}>
            <div>
              <div style={{ color:"#FF4444", fontSize:10, letterSpacing:3 }}>⚡ SPEED MATCH</div>
              <div style={{ color:"#fff", fontSize:12, fontWeight:700 }}>Match the value to its rule</div>
            </div>
            <div style={{ textAlign:"right" }}>
              <span style={{ color:"#00C896", fontSize:18, fontWeight:900 }}>{speedScore}</span>
              <span style={{ color:DIM, fontSize:12 }}> / </span>
              <span style={{ color:"#FF4444", fontSize:18, fontWeight:900 }}>{speedWrong}</span>
          ...
          {/* Value display */}
          <div style={{ background:"#111", border:`2px solid #FFD700`, borderRadius:4, padding:"28px 20px", textAlign:"center", marginBottom:16 }}>
            <div style={{ color:DIM, fontSize:10, letterSpacing:3, marginBottom:8 }}>THIS VALUE / RULE IS...</div>
            <div style={{ color:"#FFD700", fontSize:36, fontWeight:900, letterSpacing:2, lineHeight:1.2 }}>
              {card.value}
            </div>
          </div>
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:9 }}>
            {card.options.map((opt, i) => {
              let bc = BORDER, bg = CARD, tc = "#aaa";
              if (speedAnswered) {
                if (opt === card.correct) { bc="#00C896"; bg="#001a12"; tc="#00C896"; }
                else if (opt === speedSelected && opt !== card.correct) { bc="#FF4444"; bg="#1a0000"; tc="#FF4444"; }
                else { tc="#2a2a2a"; }
              }
              return (
                <button key={i} onClick={() => handleSpeedSelect(opt)}
                  style={{ background:bg, border:`2px solid ${bc}`, borderRadius:4, padding:"13px 14px", textAlign:"left", cursor:speedAnswered?"default":"pointer", fontFamily:"'Courier New', monospace", transition:"border-color 0.1s", display:"flex", justifyContent:"space-between" }}
                  onMouseEnter={e => { if(!speedAnswered) e.currentTarget.style.borderColor="#FFD700"; }}
                ...
                >
                  ...
                  {speedAnswered && opt===card.correct && <span style={{ color:"#00C896", flexShrink:0 }}>✓</span>}
                  {speedAnswered && opt===speedSelected && opt!==card.correct && <span style={{ color:"#FF4444", flexShrink:0 }}>✗</span>}
                </button>
              );
            })}
          </div>
          ...
          {timedOut && (
            <div style={{ marginTop:12, background:"#1a0000", border:"1px solid #FF4444", borderRadius:4, padding:"12px 14px" }}>
              <div style={{ color:"#FF4444", fontSize:10, letterSpacing:3 }}>⏱ TIME'S UP — CORRECT ANSWER:</div>
              <div style={{ color:"#00C896", fontSize:14, fontWeight:900, marginTop:6 }}>{card.correct}</div>
            </div>
          )}

          {speedAnswered && (
            <button onClick={nextSpeed} style={{ width:"100%", marginTop:12, padding:"13px", background:"#FF4444", color:"#fff", border:"none", borderRadius:4, fontSize:12, fontWeight:900, letterSpacing:3, cursor:"pointer", fontFamily:"'Courier New', monospace" }}>
  ...
  if (mode === "speedresult") {
    ...
    return (
      <div style={{ minHeight:"100vh", background: BG, fontFamily:"'Courier New', monospace", padding:"24px 16px", color: TEXT }}>
        <div style={{ maxWidth:640, margin:"0 auto" }}>
          <div style={{ background: pct>=75?"#001a12":"#1a0000", border:`2px solid ${pct>=75?"#00C896":"#FF4444"}`, borderRadius:4, padding:"28px", textAlign:"center", marginBottom:24 }}>
            <div style={{ color:pct>=75?"#00C896":"#FF4444", fontSize:10, letterSpacing:4, marginBottom:10 }}>SPEED MATCH RESULT</div>
            <div style={{ color:"#fff", fontSize:48, fontWeight:900, lineHeight:1 }}>
              {speedScore}<span style={{ color:"#333" }}>/{total}</span>
            </div>
            <div style={{ display:"flex", justifyContent:"center", gap:24, marginTop:14 }}>
              <div><div style={{ color:"#00C896", fontSize:24, fontWeight:900 }}>{speedScore}</div><div style={{ color:DIM, fontSize:10, letterSpacing:2 }}>CORRECT</div></div>
              <div><div style={{ color:"#FF4444", fontSize:24, fontWeight:900 }}>{speedWrong}</div><div style={{ color:DIM, fontSize:10, letterSpacing:2 }}>WRONG/TIMED</div></div>
            </div>
          ...
          <div style={{ display:"flex", gap:10 }}>
            <button onClick={startSpeed} style={{ flex:1, padding:"13px", background:"#FF4444", color:"#fff", border:"none", borderRadius:4, fontSize:11, fontWeight:900, letterSpacing:2, cursor:"pointer", fontFamily:"'Courier New', monospace" }}>RETRY</button>
            <button onClick={() => setMode("home")} style={{ flex:1, padding:"13px", background:"#FFD700", color:"#000", border:"none", borderRadius:4, fontSize:11, fontWeight:900, letterSpacing:2, cursor:"pointer", fontFamily:"'Courier New', monospace" }}>HOME</button>
          </div>
        </div>
      </div>
