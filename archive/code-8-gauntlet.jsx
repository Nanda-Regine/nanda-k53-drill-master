...
export default function Code8Gauntlet() {
  ...
  const activeColor = isExamMode ? "#FFD700" : (currentTest?.color || "#FFD700");
  ...
  if (screen === "home") {
    return (
      <div style={{ minHeight: "100vh", background: "#0a0a0a", fontFamily: "'Courier New', monospace", padding: "24px 16px" }}>
        <div style={{ maxWidth: 640, margin: "0 auto" }}>
          <div style={{ textAlign: "center", marginBottom: 32 }}>
            <div style={{ fontSize: 11, letterSpacing: 6, color: "#666", marginBottom: 8, textTransform: "uppercase" }}>South Africa</div>
            <div style={{ display: "inline-block", background: "#FFD700", color: "#000", fontSize: 36, fontWeight: 900, padding: "8px 20px", letterSpacing: -1, marginBottom: 4 }}>
              CODE 8
            </div>
            <div style={{ fontSize: 13, color: "#FFD700", letterSpacing: 4, marginTop: 8 }}>LEARNER'S GAUNTLET — 90 QUESTIONS</div>
          ...
          {(allScores.length > 0 || bestStreak > 0) && (
            <div style={{ background: "#111", border: "1px solid #222", borderRadius: 4, padding: "14px 20px", marginBottom: 24, display: "flex", justifyContent: "space-around", textAlign: "center" }}>
              <div><div style={{ color: "#FFD700", fontSize: 22, fontWeight: 900 }}>{totalScore}/{allScores.reduce((a,b)=>a+b.total,0)}</div><div style={{ color: "#444", fontSize: 10, letterSpacing: 2 }}>TOTAL</div></div>
              <div><div style={{ color: "#00C896", fontSize: 22, fontWeight: 900 }}>{bestStreak}</div><div style={{ color: "#444", fontSize: 10, letterSpacing: 2 }}>BEST STREAK</div></div>
              <div><div style={{ color: "#FF4444", fontSize: 22, fontWeight: 900 }}>{allScores.length}/9</div><div style={{ color: "#444", fontSize: 10, letterSpacing: 2 }}>ROUNDS DONE</div></div>
            </div>
          )}

          {/* EXAM MODE */}
          <div style={{ marginBottom: 20 }}>
            <div style={{ color: "#FFD700", fontSize: 10, letterSpacing: 3, marginBottom: 10 }}>⚡ EXAM SIMULATOR</div>
            <div style={{ display: "flex", gap: 10 }}>
              <button onClick={() => startExam(false)} style={{ flex: 1, background: "#FFD700", color: "#000", border: "none", borderRadius: 4, padding: "14px 10px", fontWeight: 900, fontSize: 12, letterSpacing: 2, cursor: "pointer", fontFamily: "'Courier New', monospace" }}>
                60 RANDOM Q's
              </button>
              <button onClick={() => startExam(true)} style={{ flex: 1, background: "#FF4444", color: "#fff", border: "none", borderRadius: 4, padding: "14px 10px", fontWeight: 900, fontSize: 12, letterSpacing: 2, cursor: "pointer", fontFamily: "'Courier New', monospace" }}>
                ⏱ TIMED MODE (30s/Q)
              </button>
            </div>
          </div>
          ...
          <div style={{ color: "#555", fontSize: 10, letterSpacing: 3, marginBottom: 14 }}>INDIVIDUAL ROUNDS</div>
            ...
            {TESTS.map((test, i) => {
              ...
              return (
                <button key={test.id} onClick={() => startTest(i)}
                  style={{ background: "#111", border: `2px solid ${done ? (passed ? "#00C896" : "#FF4444") : "#1e1e1e"}`, borderRadius: 4, padding: "14px 16px", textAlign: "left", cursor: "pointer", fontFamily: "'Courier New', monospace", transition: "border-color 0.15s" }}
                  onMouseEnter={(e) => e.currentTarget.style.borderColor = test.color}
                  onMouseLeave={(e) => e.currentTarget.style.borderColor = done ? (passed ? "#00C896" : "#FF4444") : "#1e1e1e"}
                >
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    <div>
                      ...
                      <div style={{ color: "#444", fontSize: 11, marginTop: 2 }}>{test.subtitle}</div>
                    </div>
                    <div style={{ textAlign: "right", minWidth: 50 }}>
                      {done ? (
                        <>
                          <div style={{ color: passed ? "#00C896" : "#FF4444", fontSize: 22, fontWeight: 900 }}>{done.score}/10</div>
                          <div style={{ color: passed ? "#00C896" : "#FF4444", fontSize: 9, letterSpacing: 2 }}>{passed ? "PASSED" : "RETRY"}</div>
                        </>
                      ) : (
          ...
          {allScores.length > 0 && (
            <button onClick={resetAll} style={{ width: "100%", marginTop: 18, padding: "12px", background: "transparent", border: "1px solid #222", color: "#444", fontSize: 11, letterSpacing: 2, cursor: "pointer", fontFamily: "'Courier New', monospace", borderRadius: 4 }}>
              RESET ALL SCORES
            </button>
          )}
          <p style={{ textAlign: "center", color: "#222", fontSize: 10, marginTop: 28, letterSpacing: 1 }}>Based on SA Road Traffic Act No. 93 of 1996</p>
        </div>
      </div>
    );
  }
  ...
  if (screen === "quiz" && currentQ) {
    ...
    return (
      <div style={{ minHeight: "100vh", background: "#0a0a0a", fontFamily: "'Courier New', monospace", padding: "20px 16px" }}>
        <div style={{ maxWidth: 640, margin: "0 auto" }}>
          {/* Header */}
          ...
          {/* Progress bar */}
          <div style={{ height: 3, background: "#1a1a1a", borderRadius: 2, marginBottom: timedMode ? 8 : 20, position: "relative" }}>
          ...
          {/* Timer bar */}
          {timedMode && (
            <div style={{ height: 4, background: "#1a1a1a", borderRadius: 2, marginBottom: 20, position: "relative" }}>
          ...
          {/* Q counter + streak */}
          <div style={{ color: "#444", fontSize: 11, letterSpacing: 3, marginBottom: 12 }}>
            Q {qIndex + 1} / {totalQ}
            {currentStreak >= 3 && <span style={{ color: "#FFD700", marginLeft: 16 }}>🔥 {currentStreak} STREAK</span>}
          </div>

          {/* Question */}
          <div style={{ background: "#111", border: "1px solid #1e1e1e", borderRadius: 4, padding: "20px", marginBottom: 14, color: "#fff", fontSize: 14, lineHeight: 1.7, fontWeight: 600 }}>
            {currentQ.q}
          </div>
            ...
            {currentQ.options.map((opt, i) => {
              ...
              return (
                <button key={i} onClick={() => handleSelect(i)}
                  style={{ background: bg, border: `2px solid ${bc}`, borderRadius: 4, padding: "13px 14px", textAlign: "left", cursor: answered ? "default" : "pointer", display: "flex", gap: 10, alignItems: "flex-start", transition: "border-color 0.1s", fontFamily: "'Courier New', monospace" }}
                  onMouseEnter={(e) => { if (!answered) e.currentTarget.style.borderColor = activeColor; }}
                  onMouseLeave={(e) => { if (!answered) e.currentTarget.style.borderColor = "#1e1e1e"; }}
                >
          ...
          {answered && (
            <button onClick={handleNext} style={{ width: "100%", padding: "13px", background: activeColor, color: "#000", border: "none", borderRadius: 4, fontSize: 12, fontWeight: 900, letterSpacing: 3, cursor: "pointer", fontFamily: "'Courier New', monospace" }}>
        ...
        <div style={{ maxWidth: 640, margin: "0 auto" }}>
          ...
          <div style={{ background: passed ? "#001a12" : "#1a0000", border: `2px solid ${passed ? "#00C896" : "#FF4444"}`, borderRadius: 4, padding: "28px 24px", textAlign: "center", marginBottom: 24 }}>
            ...
            <div style={{ color: "#fff", fontSize: 56, fontWeight: 900, lineHeight: 1 }}>
              {score}<span style={{ color: "#222" }}>/{finalTotal}</span>
            </div>
            <div style={{ color: passed ? "#00C896" : "#FF4444", fontSize: 20, fontWeight: 900, marginTop: 6 }}>
              {pct}%
            </div>
          ...
          {wrongAnswers.length > 0 && (
            <div style={{ marginBottom: 24 }}>
              <div style={{ color: "#FF4444", fontSize: 10, letterSpacing: 3, marginBottom: 14 }}>✗ REVIEW YOUR MISTAKES ({wrongAnswers.length})</div>
              <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                {wrongAnswers.map((w, i) => (
                  <div key={i} style={{ background: "#111", border: "1px solid #2a0000", borderRadius: 4, padding: "14px" }}>
                    ...
                    <div style={{ color: "#555", fontSize: 11, lineHeight: 1.5, borderTop: "1px solid #1a1a1a", paddingTop: 8 }}>{w.explain}</div>
                  </div>
                ))}
              </div>
            </div>
          ...
          {wrongAnswers.length === 0 && (
            <div style={{ background: "#001a12", border: "1px solid #00C896", borderRadius: 4, padding: 20, textAlign: "center", marginBottom: 24 }}>
              <div style={{ color: "#00C896", fontSize: 16 }}>🔥 PERFECT ROUND — Not a single mistake.</div>
            </div>
          )}
          ...
          <div style={{ display: "flex", gap: 10 }}>
            <button onClick={() => isExamMode ? startExam(timedMode) : startTest(testIndex)}
              style={{ flex: 1, padding: "13px", background: "#FF4444", color: "#fff", border: "none", borderRadius: 4, fontSize: 11, fontWeight: 900, letterSpacing: 2, cursor: "pointer", fontFamily: "'Courier New', monospace" }}>
              RETRY
            </button>
            <button onClick={() => setScreen("home")}
              style={{ flex: 1, padding: "13px", background: "#FFD700", color: "#000", border: "none", borderRadius: 4, fontSize: 11, fontWeight: 900, letterSpacing: 2, cursor: "pointer", fontFamily: "'Courier New', monospace" }}>
              HOME →
            </button>
          </div>
          ...
          {!isExamMode && allScores.length === 9 && (
            <div style={{ marginTop: 22, background: "#111", border: "1px solid #222", borderRadius: 4, padding: "20px", textAlign: "center" }}>
              <div style={{ color: "#FFD700", fontSize: 10, letterSpacing: 3, marginBottom: 8 }}>ALL 9 ROUNDS COMPLETE</div>
