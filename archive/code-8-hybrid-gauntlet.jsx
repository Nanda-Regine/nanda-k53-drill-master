...
export default function UltimateGauntlet() {
  ...
  const activeColor = isExamMode ? "#FF0066" : (currentTest?.color || "#FF0066");
  ...
  if (screen === "home") return (
    <div style={{ minHeight: "100vh", background: "#050505", fontFamily: "'Courier New', monospace", padding: "24px 16px" }}>
      <div style={{ maxWidth: 640, margin: "0 auto" }}>
        <div style={{ textAlign: "center", marginBottom: 32 }}>
          <div style={{ fontSize: 10, letterSpacing: 6, color: "#444", marginBottom: 10 }}>TEST DAY EVE — FINAL BOSS</div>
          <div style={{ display: "inline-block", background: "#FF0066", color: "#fff", fontSize: 28, fontWeight: 900, padding: "10px 24px", letterSpacing: -1, marginBottom: 4 }}>HYBRID GAUNTLET</div>
          <div style={{ fontSize: 11, color: "#FF0066", letterSpacing: 4, marginTop: 8 }}>100 QUESTIONS — SIGNS + RULES — MAXIMUM DIFFICULTY</div>
          <p style={{ color: "#444", fontSize: 12, marginTop: 14, lineHeight: 1.8 }}>
            Every trap. Every exception. Every "EXCEPT" question.<br />
            Signs law + Rules of the Road fused together.<br />
            <span style={{ color: "#FF0066", fontWeight: 900 }}>If you can crack this, you WILL pass tomorrow.</span>
          </p>
        </div>

        {(allScores.length > 0 || bestStreak > 0) && (
          <div style={{ background: "#0f0f0f", border: "1px solid #1a1a1a", borderRadius: 4, padding: "14px 20px", marginBottom: 24, display: "flex", justifyContent: "space-around", textAlign: "center" }}>
            <div><div style={{ color: "#FF0066", fontSize: 22, fontWeight: 900 }}>{totalScore}/{allScores.reduce((a,b)=>a+b.total,0)}</div><div style={{ color: "#333", fontSize: 10, letterSpacing: 2 }}>TOTAL</div></div>
            <div><div style={{ color: "#00C896", fontSize: 22, fontWeight: 900 }}>{bestStreak}</div><div style={{ color: "#333", fontSize: 10, letterSpacing: 2 }}>BEST STREAK</div></div>
            <div><div style={{ color: "#FFD700", fontSize: 22, fontWeight: 900 }}>{allScores.length}/10</div><div style={{ color: "#333", fontSize: 10, letterSpacing: 2 }}>ROUNDS DONE</div></div>
          </div>
        )}

        {/* Exam simulator */}
        <div style={{ marginBottom: 20 }}>
          <div style={{ color: "#FF0066", fontSize: 10, letterSpacing: 3, marginBottom: 10 }}>⚡ FULL EXAM SIMULATOR — 70 RANDOM QUESTIONS</div>
          <div style={{ display: "flex", gap: 10 }}>
            <button onClick={() => startExam(false)} style={{ flex: 1, background: "#FF0066", color: "#fff", border: "none", borderRadius: 4, padding: "16px", fontWeight: 900, fontSize: 12, letterSpacing: 2, cursor: "pointer", fontFamily: "'Courier New', monospace" }}>
              70 RANDOM Q's
            </button>
            <button onClick={() => startExam(true)} style={{ flex: 1, background: "#111", color: "#FF0066", border: "2px solid #FF0066", borderRadius: 4, padding: "16px", fontWeight: 900, fontSize: 12, letterSpacing: 2, cursor: "pointer", fontFamily: "'Courier New', monospace" }}>
              ⏱ TIMED (30s/Q)
            </button>
          </div>
        </div>
        ...
        <div style={{ color: "#333", fontSize: 10, letterSpacing: 3, marginBottom: 14 }}>10 ROUNDS — DRILL BY TOPIC</div>
          ...
          {TESTS.map((test, i) => {
            ...
            return (
              <button key={test.id} onClick={() => startTest(i)}
                style={{ background: "#0f0f0f", border: `2px solid ${done ? (passed ? "#00C896" : "#FF0066") : "#1a1a1a"}`, borderRadius: 4, padding: "14px 16px", textAlign: "left", cursor: "pointer", fontFamily: "'Courier New', monospace" }}
                onMouseEnter={e => e.currentTarget.style.borderColor = test.color}
                onMouseLeave={e => e.currentTarget.style.borderColor = done ? (passed ? "#00C896" : "#FF0066") : "#1a1a1a"}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  <div>
                    ...
                    <div style={{ color: "#333", fontSize: 11, marginTop: 2 }}>{test.subtitle}</div>
                  </div>
                  <div style={{ textAlign: "right", minWidth: 50 }}>
                    {done ? (
                      <><div style={{ color: passed ? "#00C896" : "#FF0066", fontSize: 22, fontWeight: 900 }}>{done.score}/10</div><div style={{ color: passed ? "#00C896" : "#FF0066", fontSize: 9, letterSpacing: 2 }}>{passed ? "PASSED" : "RETRY"}</div></>
                    ) : <div style={{ color: "#1a1a1a", fontSize: 22, fontWeight: 900 }}>—</div>}
                  </div>
                </div>
              </button>
            );
        ...
        {allScores.length > 0 && (
          <button onClick={resetAll} style={{ width: "100%", marginTop: 18, padding: "12px", background: "transparent", border: "1px solid #1a1a1a", color: "#333", fontSize: 11, letterSpacing: 2, cursor: "pointer", fontFamily: "'Courier New', monospace", borderRadius: 4 }}>
            RESET ALL SCORES
          </button>
        )}

        <div style={{ marginTop: 28, background: "#0f0f0f", border: "1px solid #1a1a1a", borderRadius: 4, padding: "16px" }}>
          <div style={{ color: "#FF0066", fontSize: 10, letterSpacing: 3, marginBottom: 12 }}>🔥 TEST DAY REMINDERS</div>
          {["Read EVERY question TWICE — especially anything with 'NOT', 'EXCEPT', 'UNLESS'","'Exceeds' ≠ 'equals' — exactly 9,000 kg GVM is NOT subject to 80 km/h","'Or more' means the number itself is an offence — 0.05g IS an offence","Yield sign = triangular BUT regulatory. Traffic officer BEATS all signals.","Dipped = 45m. Main beam = 100m. Stop lamp = 30m. Hooter = 90m.","Pedestrian crossing = 9m. Intersection = 5m. Tunnel = 6m.","Tow rope: 30 km/h max, 3.5m max. Draw-bar: no speed restriction.","Motor cars do NOT need a warning triangle. Motorcycles may NOT tow.","Disc grace = 21 days. Address change = 14 days. Accident report = 24 hrs."].map((tip, i) => (
            <div key={i} style={{ color: "#555", fontSize: 11, lineHeight: 1.6, paddingBottom: 6, marginBottom: 6, borderBottom: i < 8 ? "1px solid #111" : "none" }}>
              <span style={{ color: "#FF0066", marginRight: 8 }}>▸</span>{tip}
            </div>
          ))}
        </div>
        <p style={{ textAlign: "center", color: "#1a1a1a", fontSize: 10, marginTop: 24 }}>You've got this. 8/9 means you're ready. Go get that licence. 🏆</p>
      </div>
    </div>
  );
  ...
  if (screen === "quiz" && currentQ) {
    ...
    const timerColor = timeLeft > 15 ? "#00C896" : timeLeft > 7 ? "#FFD700" : "#FF4444";
    return (
      <div style={{ minHeight: "100vh", background: "#050505", fontFamily: "'Courier New', monospace", padding: "20px 16px" }}>
        <div style={{ maxWidth: 640, margin: "0 auto" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
            ...
            <div style={{ textAlign: "right" }}>
              ...
              <div style={{ color: "#222", fontSize: 10, letterSpacing: 2 }}>SCORE</div>
            </div>
          </div>

          <div style={{ height: 3, background: "#111", borderRadius: 2, marginBottom: timedMode ? 6 : 18 }}>
            <div style={{ height: "100%", width: `${progress}%`, background: activeColor, borderRadius: 2, transition: "width 0.3s" }} />
          </div>

          {timedMode && (
            <div style={{ height: 4, background: "#111", borderRadius: 2, marginBottom: 16, position: "relative" }}>
          ...
          <div style={{ color: "#333", fontSize: 11, letterSpacing: 3, marginBottom: 12 }}>
            Q {qIndex + 1} / {totalQ}
            {currentStreak >= 3 && <span style={{ color: "#FFD700", marginLeft: 16 }}>🔥 {currentStreak} STREAK</span>}
          </div>

          <div style={{ background: "#0f0f0f", border: `1px solid ${activeColor}33`, borderRadius: 4, padding: "20px", marginBottom: 14, color: "#fff", fontSize: 14, lineHeight: 1.8, fontWeight: 600 }}>
            {currentQ.q}
          </div>

          <div style={{ display: "flex", flexDirection: "column", gap: 9, marginBottom: 14 }}>
            {currentQ.options.map((opt, i) => {
              let bc = "#1a1a1a", bg = "#0f0f0f", tc = "#888";
              if (answered) {
                if (i === currentQ.answer) { bc = "#00C896"; bg = "#001a12"; tc = "#00C896"; }
                else if (i === selected) { bc = "#FF4444"; bg = "#1a0000"; tc = "#FF4444"; }
                else { tc = "#222"; }
              }
              return (
                <button key={i} onClick={() => handleSelect(i)}
                  style={{ background: bg, border: `2px solid ${bc}`, borderRadius: 4, padding: "13px 14px", textAlign: "left", cursor: answered ? "default" : "pointer", display: "flex", gap: 10, alignItems: "flex-start", fontFamily: "'Courier New', monospace", transition: "border-color 0.1s" }}
                  onMouseEnter={e => { if (!answered) e.currentTarget.style.borderColor = activeColor; }}
                  onMouseLeave={e => { if (!answered) e.currentTarget.style.borderColor = "#1a1a1a"; }}>
                  ...
                  {answered && i === currentQ.answer && <span style={{ marginLeft: "auto", color: "#00C896" }}>✓</span>}
                  {answered && i === selected && i !== currentQ.answer && <span style={{ marginLeft: "auto", color: "#FF4444" }}>✗</span>}
                </button>
              );
            })}
          </div>
          ...
          {showExplain && (
            <div style={{ background: isCorrect ? "#001a12" : "#1a0000", border: `1px solid ${isCorrect ? "#00C896" : "#FF4444"}`, borderRadius: 4, padding: "14px 16px", marginBottom: 14 }}>
              <div style={{ color: isCorrect ? "#00C896" : "#FF4444", fontSize: 10, letterSpacing: 3, marginBottom: 8, fontWeight: 900 }}>
          ...
          {answered && (
            <button onClick={handleNext} style={{ width: "100%", padding: "14px", background: activeColor, color: "#fff", border: "none", borderRadius: 4, fontSize: 12, fontWeight: 900, letterSpacing: 3, cursor: "pointer", fontFamily: "'Courier New', monospace" }}>
  ...
  if (screen === "result") {
    ...
    return (
      <div style={{ minHeight: "100vh", background: "#050505", fontFamily: "'Courier New', monospace", padding: "24px 16px" }}>
        <div style={{ maxWidth: 640, margin: "0 auto" }}>
          <div style={{ background: passed ? "#001a12" : "#1a0000", border: `2px solid ${passed ? "#00C896" : "#FF4444"}`, borderRadius: 4, padding: "28px 24px", textAlign: "center", marginBottom: 24 }}>
            <div style={{ color: passed ? "#00C896" : "#FF4444", fontSize: 10, letterSpacing: 4, marginBottom: 10 }}>
              {isExamMode ? "HYBRID EXAM RESULT" : `ROUND ${currentTest?.id} RESULT`}
            </div>
            <div style={{ color: "#fff", fontSize: 56, fontWeight: 900, lineHeight: 1 }}>
              {score}<span style={{ color: "#1a1a1a" }}>/{finalTotal}</span>
            </div>
            <div style={{ color: passed ? "#00C896" : "#FF4444", fontSize: 22, fontWeight: 900, marginTop: 6 }}>{pct}%</div>
          ...
          {wrongAnswers.length > 0 && (
            <div style={{ marginBottom: 24 }}>
              <div style={{ color: "#FF4444", fontSize: 10, letterSpacing: 3, marginBottom: 14 }}>✗ MISTAKES TO MEMORISE TONIGHT ({wrongAnswers.length})</div>
              {wrongAnswers.map((w, i) => (
                <div key={i} style={{ background: "#0f0f0f", border: "1px solid #2a0000", borderRadius: 4, padding: "14px", marginBottom: 10 }}>
                  <div style={{ color: "#555", fontSize: 12, marginBottom: 8, lineHeight: 1.5 }}>{w.q}</div>
                  <div style={{ color: "#FF4444", fontSize: 11, marginBottom: 4 }}>✗ {w.yourAnswer}</div>
                  <div style={{ color: "#00C896", fontSize: 11, marginBottom: 8 }}>✓ {w.correctAnswer}</div>
                  <div style={{ color: "#444", fontSize: 11, lineHeight: 1.6, borderTop: "1px solid #111", paddingTop: 8 }}>{w.explain}</div>
                </div>
              ))}
            </div>
          )}
          ...
          {wrongAnswers.length === 0 && (
            <div style={{ background: "#001a12", border: "1px solid #00C896", borderRadius: 4, padding: 20, textAlign: "center", marginBottom: 24 }}>
              <div style={{ color: "#00C896", fontSize: 18 }}>🔥 PERFECT. You are not human. Go sleep.</div>
            </div>
          )}

          <div style={{ display: "flex", gap: 10 }}>
            <button onClick={() => isExamMode ? startExam(timedMode) : startTest(testIndex)} style={{ flex: 1, padding: "13px", background: "#FF0066", color: "#fff", border: "none", borderRadius: 4, fontSize: 11, fontWeight: 900, letterSpacing: 2, cursor: "pointer", fontFamily: "'Courier New', monospace" }}>RETRY</button>
            <button onClick={() => setScreen("home")} style={{ flex: 1, padding: "13px", background: "#FFD700", color: "#000", border: "none", borderRadius: 4, fontSize: 11, fontWeight: 900, letterSpacing: 2, cursor: "pointer", fontFamily: "'Courier New', monospace" }}>HOME →</button>
          </div>

          {!isExamMode && allScores.length === 10 && (
            <div style={{ marginTop: 22, background: "#0f0f0f", border: "1px solid #1a1a1a", borderRadius: 4, padding: "20px", textAlign: "center" }}>
              <div style={{ color: "#FF0066", fontSize: 10, letterSpacing: 3, marginBottom: 8 }}>ALL 10 ROUNDS COMPLETE</div>
